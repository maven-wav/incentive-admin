...
- **시책 유형** = `incentive_claims.type` (가맹모집 · 비가맹결제건수 · 얼라이언스QR부착 · 홍보물부착). 위 모집유형과 **완전히 별개 필드**다.
  - **가맹모집**은 가맹 계열 등록 시 자동 생성(접수 화면엔 없음), 나머지 3종이 대리점 시책 접수 대상 (`SUBMITTABLE_CLAIM_TYPES`).
...
3요소로 판정한다:
1. **가맹 여부** — 가맹선불·가맹후불·**PG선불**이 '가맹'(2026-07-24 R2 전제 P1로 PG선불 포함). 비가맹선불·비가맹후불만 비가맹.
...
- [x] **R2** 가맹 계열 등록=승인 스킵 + 가맹모집 시책 자동생성(백그라운드로 시책검수 접수), 비가맹은 승인 유지
- [x] **R2** 시책 접수 3유형(비가맹결제건수·얼라이언스QR부착·홍보물부착) + `claim_type` enum 2종 추가
- [x] **R2** 기등록 CID 차단(반려 제외) · 반려/삭제 시 재등록 허용 · 내부 담당자 행 삭제(CASCADE, 지급확정도 강한 경고 후 삭제 가능)
- [x] **R2** 동일 사업자번호 안내(등록) · 비가맹 확인 모달 · 등록내역 ⚠️ 같은 사업자 뱃지/목록
- [x] **R2** 대리점 '시책 내역'(`/incentives/mine`) — 내 시책 조회 + 검수대기 건 자가 철회(`withdrawClaim`)
- [x] **R2** 권한-이동 정합성: 접수 성공/취소·가맹 등록·대시보드 링크 모두 로그인 주체가 접근 가능한 페이지로만 이동
      (대리점은 시책검수 대신 시책 내역/등록내역으로)
...
## ⚠️ 임시값 (실정책으로 교체)
- 시책 단가: 가맹모집·비가맹결제건수 30,000 / 얼라이언스QR부착·홍보물부착 20,000 (`lib/mockData.js`, 비가맹결제건수는 P2 임시)
- 결제검수 기준: 3개월 **총** 결제 4건 이상 (`TXN_TOTAL_CRITERIA`)
- **PG선불 = 가맹으로 통일** (R2 전제 P1: `hasCid`=`isGamaeng`=가맹선불·가맹후불·PG선불). 정책 확정 시 재검토.
- 비가맹결제건수 입력 주체 = 대리점(접수 시 입력), 담당자는 검수에서 확인 (P3, 미확정).
- 얼라이언스QR "20개 이상" — 현재 사진+안내문구만. 개수 검증 필요 시 컬럼 추가.
- 대리점 계좌번호는 전부 placeholder
...
## 🔔 2026-07-24 리뷰 수정 R2 (먼저 볼 것)
> 라운드 1 이후 나온 수정 11건은 **`HANDOFF_리뷰수정_2026-07-24_R2.md`** 에 정리됨.
> 핵심: ⑥ 가맹 등록=승인 스킵+자동 시책검수 / ⑦ 비가맹은 승인 유지 / ⑧ 시책 접수 3유형(비가맹결제건수·얼라이언스QR부착·홍보물부착, 가맹모집 제외) / ② 기등록 CID 차단+③ 반려·삭제 시 재등록 / ④ 내부 담당자 행 삭제 / ⑨~⑪ 동일 사업자번호 안내·경고. claim_type enum 2종 추가 SQL 포함. **전제: PG선불=가맹으로 통일**.
