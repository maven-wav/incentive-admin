...
import { MERCHANT_STATUS, CLAIM_STATUS, CLAIM_TYPES, INCENTIVE_AMOUNT, hasCid, PHOTO_CLAIM_TYPES } from "./mockData";
...
export function StoreProvider({ children }) {
  ...
  // ── 가맹점 액션 ────────────────────────────────────────────
  // 가맹 계열(hasCid)은 등록 시점에 카카오페이가 가맹/활성화/거래건수를 확인하므로
  // 담당자 승인을 스킵하고 바로 승인완료 + 가맹모집 시책을 자동 생성한다 → 시책검수로 직행.
  // 비가맹은 기존대로 등록대기 → 담당자 승인.
  const registerMerchant = useCallback(
    async (data) => {
      ...
      if (gamaeng) {
        // 가맹모집 시책 자동 생성 → 검수대기로 바로 진입 (실연동 시 콰트로가 활성화·건수 채움)
        const { data: crow, error: cerr } = await supabase
          ...
          .insert({
            ...
            type: CLAIM_TYPES.RECRUIT,
            claim_month: today().slice(0, 7),
            status: CLAIM_STATUS.REVIEW,
            is_active: false,
            txn_total: 0,
            proof_photo: null,
            amount: INCENTIVE_AMOUNT[CLAIM_TYPES.RECRUIT],
          })
          .select()
          .single();
        if (cerr) {
          fail(cerr, "가맹 시책 자동생성");
        } else {
  ...
  // ── 시책 액션 ──────────────────────────────────────────────
  // 접수 가능한 유형: 비가맹결제건수 · 얼라이언스QR부착 · 홍보물부착 (가맹모집은 등록 시 자동생성).
  const submitClaim = useCallback(
    async ({ merchantId, agencyId, type, claimMonth, proofPhoto, txnTotal }) => {
  ...
  // ── 삭제 (내부 전용) ───────────────────────────────────────
  // merchants FK가 ON DELETE CASCADE라 삭제 시 연결된 claims·history도 함께 사라진다.
  ...
  // 대리점 자가 철회 — 검수대기 상태에서만. 담당자 검토 이후(정산대기·지급확정·반려)는 이력 보존.
  const withdrawClaim = useCallback(
    async (id) => {
      const c = claims.find((x) => x.id === id);
      if (!c || c.status !== CLAIM_STATUS.REVIEW) {
        flash("검수대기 상태에서만 철회할 수 있습니다.");
        return;
      }
      const { error } = await supabase
        .from("incentive_claims")
        .delete()
        .eq("id", id)
        .eq("status", CLAIM_STATUS.REVIEW); // 경합 방지: 검수대기일 때만
  ...
  const value = {
    ...
    deleteMerchant, deleteClaim, withdrawClaim,
  };
